 ...
 *
 * Four policies, all over ninety days of Ethereum mainnet (648,000 blocks at 12s). `BlankFile` is the
 * default and appears first: silence is silence; a refutation or a listed event inside the window
 * blocks, and so does an open claim. The one `BondedClean` policy needs a standing no-event claim that
 * covers the whole window, ends within a week of the head, and whose *unrecoverable* half covers the
 * principal -- the expensive kind of clean, and never the default.
 *
  ...
  for (const w of wanted) {
    const dup = existing.findIndex(
      (p: any) =>
        ...
        p.topic0 === w.venue.topic0 && BigInt(p.minBond) === w.minBond,
    );
    if (dup >= 0) {
